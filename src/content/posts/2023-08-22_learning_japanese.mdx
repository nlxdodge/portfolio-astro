...
For example the letters `ち` and `ら` look alike in DuoLingo, just as `は` and `ほ` but they are in fact way different.
For people outside Germanic languages this can be seen the same as `i` and `j`. They look alike but are way different from what they mean and how the language works. 
But can be hard for first time learners trying to get a grasp on all the characters in the alphabet.
